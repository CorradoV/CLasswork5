...
#include <kdl_parser/kdl_parser.hpp>           //include external libraries, we can not use only ros
...
bool KUKA_INVKIN::init_robot_model() {
	...
	//Retrieve the robot description (URDF) from the robot_description param  (i have to start launch file)
...
void KUKA_INVKIN::ctrl_loop() {
	...
	int N=200; //number of samples (duration 4 s)
	float t=0.0;
	...
	pi<<_p_out.p.x(),_p_out.p.y(),_p_out.p.z();
	pf<<0.7,0.3,0.4;
	VectorXd diff=pf-pi;
	float leng=diff.norm();
	//cout<<"Norma vettori: "<<leng<<endl;
	float a_3=-0.016238;
	float a_2=0.097427;
	MatrixXd pdes(3,N);
	float s; //arc lenght
	int k=1; //colunm entry
	//calculation of the trajectory
	while(k<=200){
		s=a_3*pow(t,3)+a_2*pow(t,2);
		curr=pi+(s/leng)*diff;
		pdes(0,k-1)=curr(0);
		pdes(1,k-1)=curr(1);
		pdes(2,k-1)=curr(2);
	...
	string path = ros::package::getPath("classwork5");
	string path1=path+"/EE_trajectory.txt";
	string path2=path+"/q_trajectory.txt";
	string path3=path+"/time.txt";
	...
	EE_data<<_p_out.p.x()<<",";
	EE_data<<_p_out.p.y()<<",";
	EE_data<<_p_out.p.z()<<endl;

	while(k<=200){


	F_dest.p.data[0] = pdes(0,k-1);
	F_dest.p.data[1] = pdes(1,k-1);
	F_dest.p.data[2] = pdes(2,k-1);
	

	//The orientation set point is the same of the current one
	for(int j=0; j<9; j++ )
		F_dest.M.data[j] = _p_out.M.data[j];
	...
	for(int i=0; i<6; i++) {
		...
		cmd[6].data = q_out.data[6];
		q_data<<cmd[6].data<<endl;
	...
	cout<<"Traiettoria terminata "<<endl;
	cout<<"istante finale: "<<t<<endl;
